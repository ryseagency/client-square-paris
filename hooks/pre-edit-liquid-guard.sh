...
# ────────────────────────────────────────────────────────────────────────────
# RÈGLE 18 — schema avec presets ET default simultanés — bug A3
# ────────────────────────────────────────────────────────────────────────────
# Theme check ne le détecte pas, mais theme dev plante. Choisir UN des deux.
both_violations=$(
  REASON_INPUT="$content" python3 <<'PYEOF' 2>/dev/null
import os, json, re
raw = os.environ.get("REASON_INPUT", "")
...
if "presets" in schema and "default" in schema:
    print("schema contient `presets` ET `default` simultanés (Shopify rejette à theme dev) — choisir un seul")
PYEOF
)
if [ -n "$both_violations" ]; then
  while IFS= read -r line; do
    violations+=("Schema presets+default simultanés : $line")
  done <<< "$both_violations"
fi

# ────────────────────────────────────────────────────────────────────────────
# RÈGLE 19 — metafield setting type avec options non-array — bug A19
# ────────────────────────────────────────────────────────────────────────────
# Bug GitHub #2022 : metafield settings.options doit être array, pas object.
metafield_options_violations=$(
  REASON_INPUT="$content" python3 <<'PYEOF' 2>/dev/null
import os, json, re
raw = os.environ.get("REASON_INPUT", "")
m = re.search(r"\{%\s*schema\s*%\}(.*?)\{%\s*endschema\s*%\}", raw, re.DOTALL)
...
def walk(settings, prefix):
    if not isinstance(settings, list):
        return
    for i, s in enumerate(settings):
        if not isinstance(s, dict) or s.get("type") != "metafield":
            continue
        opts = s.get("options")
        if opts is not None and not isinstance(opts, list):
            sid = s.get("id", "?")
            violations.append(f"{prefix}.settings[{i}](id={sid}) metafield options doit être array, est {type(opts).__name__}")
...
if [ -n "$metafield_options_violations" ]; then
  while IFS= read -r line; do
    violations+=("Metafield setting options non-array : $line")
  done <<< "$metafield_options_violations"
fi

# ════════════════════════════════════════════════════════════════════════════
# RÈGLES 20-22 — Pour fichiers JSON (templates/*.json) — bugs A6, A11
# ════════════════════════════════════════════════════════════════════════════

# RÈGLE 20 — block type référencé dans static blocks template JSON sans fichier — bug A6
if [[ "$file_path" == *templates/*.json ]]; then
  json_block_violations=$(
    REASON_INPUT="$content" FILE_PATH="$file_path" python3 <<'PYEOF' 2>/dev/null
import os, json
raw = os.environ.get("REASON_INPUT", "")
fp = os.environ.get("FILE_PATH", "")
...
# Trouver le theme dir (ancestor avec config/settings_schema.json)
theme_dir = os.path.dirname(fp)
while theme_dir and theme_dir != "/" and not os.path.exists(os.path.join(theme_dir, "config/settings_schema.json")):
    theme_dir = os.path.dirname(theme_dir)
if not theme_dir or theme_dir == "/":
    raise SystemExit(0)
...
def walk(obj, path=""):
    if isinstance(obj, dict):
        if "type" in obj and isinstance(obj["type"], str):
            t = obj["type"]
            if t and not t.startswith("@") and not any(t == w for w in ("section",)):
                # Cherche fichier blocks/{t}.liquid OU sections/{t}.liquid
                if not (os.path.exists(f"{theme_dir}/blocks/{t}.liquid") or os.path.exists(f"{theme_dir}/sections/{t}.liquid")):
                    if t.startswith("_") or "blocks" in path:
                        # block type
                        violations.append(f"{path}.type='{t}' référencé mais blocks/{t}.liquid + sections/{t}.liquid absents")
...
PYEOF
  ...
  if [ -n "$json_block_violations" ]; then
    while IFS= read -r line; do
      violations+=("JSONMissingBlock (cascade Shopify) : $line")
    done <<< "$json_block_violations"
  fi
fi
