...
// Shopify CSS imports - preserved from rendered theme `Square Ryse` (theme ID 198831964502)
// Source: https://udjskw-up.myshopify.com/products/yoko (HTML hydrated via Playwright, 2026-05-07)
import '@/styles/shopify-sync/base.css';
import '@/styles/shopify-sync/sq-reveal.css';
import '@/styles/shopify-sync/component-price.css';
import '@/styles/shopify-sync/component-product-variant-picker.css';
import '@/styles/shopify-sync/component-swatch-input.css';
import '@/styles/shopify-sync/component-swatch.css';
import '@/styles/shopify-sync/section-product-page-inline.css';

// Shopify CDN images - downloaded locally (src/assets/shopify-sync/)
...
/**
 * ProductPageV3 - port pixel-perfect du DOM Shopify rendu live (HTML HYDRATE).
 *
 * Source : https://udjskw-up.myshopify.com/products/yoko (theme `Square Ryse`)
 * Capture : Playwright (chromium headless 1440x900) qui execute le JavaScript Shopify
 * et capture le DOM apres hydratation des Web Components <variant-selects>,
 * <product-info>, <cart-drawer>. Le port precedent via curl rendait les options
 * de couleur en TEXTE; cette version capture les vraies pastilles visuelles
 * (mps__color-dot avec background rgb() inline).
 *
 * Note : les sections `croquis-v2` et `complementary-products` apparaissent vides
 * cote Shopify live (metafield product.metafields.custom.croquis non defini sur
 * le store de preview, et 0 blocs configures pour complementary). Le port refletera
 * fidelement ce qu'affiche le store reel - non une recreation manuelle.
 *
 * Boutons et forms neutralises (action="#") - rendu visuel uniquement, pas de logique panier.
 * Header / Footer / ReassuranceBanner Lovable conserves a la place du chrome Shopify.
 */

const ProductPageV3 = () => {
  const noopSubmit = (e: React.FormEvent) => e.preventDefault();
  return (
    <div className="bg-white min-h-screen" data-shopify-port="yoko-playwright">
      <ReassuranceBanner />
      <Header isDark={false} isHero={false} />
      <main onSubmitCapture={noopSubmit}>
